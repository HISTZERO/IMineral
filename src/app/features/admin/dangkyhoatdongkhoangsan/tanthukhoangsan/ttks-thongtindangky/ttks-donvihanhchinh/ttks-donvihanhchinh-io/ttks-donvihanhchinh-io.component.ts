...
@Component({
  ...
  templateUrl: './ttks-donvihanhchinh-io.component.html',
  styleUrls: ['./ttks-donvihanhchinh-io.component.scss']
})
export class TtksDonvihanhchinhIoComponent implements OnInit {

  // Chứa dữ liệu Form
  public dangKyTanThuDvhcIOForm: FormGroup;
  ...
  // Chứa dữ liệu input
  public inputModel: InputDkTanThuDvhc;
  ...
  async bindingConfigAddOrUpdate() {
    ...
    this.editMode = false;
    this.inputModel = new InputDkTanThuDvhc();
    // check edit
   ...
   */
  formInit() {
    this.dangKyTanThuDvhcIOForm = this.formBuilder.group({
  ...
  async formOnEdit() {
    if (this.obj && this.purpose === 'edit') {
      ...
      this.dangKyTanThuDvhcIOForm.setValue({
   ...
   */
  async showDvhcHuyen() {
    if (!this.dangKyTanThuDvhcIOForm.value.tinhcombobox) {
      this.allHuyen = [];
      this.dvhcDistrictFilters = [];
      this.allXa = [];
      this.dvhcWardFilters = [];
      if (this.editMode === true) {
        this.dangKyTanThuDvhcIOForm.controls.huyencombobox.setValue("");
      }
    }
    if (this.dangKyTanThuDvhcIOForm.value.tinhcombobox) {
      if (this.editMode === true) {
      ...
      this.allHuyen = await this.dmFacadeService
        .getDistrictService()
        .getFetchAll({IdTinh: this.dangKyTanThuDvhcIOForm.value.tinhcombobox.idtinh, Trangthai: TrangThaiEnum.Active});
   ...
   */
  async showDvhcXa() {
    if (!this.dangKyTanThuDvhcIOForm.value.huyencombobox) {
      this.allXa = [];
      this.dvhcWardFilters = [];
      if (this.editMode === true) {
        this.dangKyTanThuDvhcIOForm.controls.xacombobox.setValue("");
      }
    }
    if (this.dangKyTanThuDvhcIOForm.value.tinhcombobox && this.dangKyTanThuDvhcIOForm.value.huyencombobox) {
      if (this.editMode === true) {
        this.dangKyTanThuDvhcIOForm.controls.xacombobox.setValue("");
      }
      this.allXa = await this.dmFacadeService
        .getWardService()
        .getFetchAll({
          IdHuyen: this.dangKyTanThuDvhcIOForm.value.huyencombobox.idhuyen,
  ...
  selectTinh() {
    if (this.obj && this.purpose === 'edit') {
      if (this.dangKyTanThuDvhcIOForm.value.tinhcombobox) {
        this.dangKyTanThuDvhcIOForm.controls.tinh.setValue({
          idtinh: this.dangKyTanThuDvhcIOForm.value.tinhcombobox.idtinh,
          matinh: this.dangKyTanThuDvhcIOForm.value.tinhcombobox.matinh
        });
        this.tenTinhDisplay = this.dangKyTanThuDvhcIOForm.value.tinhcombobox.tentinh;
        this.dangKyTanThuDvhcIOForm.controls.huyen.setValue("");
        this.tenHuyenDisplay = "";
        this.dangKyTanThuDvhcIOForm.controls.xa.setValue("");
        this.tenXaDisplay = "";
      } else {
        this.dangKyTanThuDvhcIOForm.controls.tinh.setValue({
  ...
  selectHuyen() {
    if (this.obj && this.purpose === 'edit') {
      if (this.dangKyTanThuDvhcIOForm.value.huyencombobox) {
        this.dangKyTanThuDvhcIOForm.controls.huyen.setValue({
          idhuyen: this.dangKyTanThuDvhcIOForm.value.huyencombobox.idhuyen,
          mahuyen: this.dangKyTanThuDvhcIOForm.value.huyencombobox.mahuyen
        });
        this.tenHuyenDisplay = this.dangKyTanThuDvhcIOForm.value.huyencombobox.tenhuyen;
        this.dangKyTanThuDvhcIOForm.controls.xa.setValue("");
        this.tenXaDisplay = "";
      } else {
        if (this.dangKyTanThuDvhcIOForm.value.tinhcombobox) {
  ...
  selectXa() {
    if (this.obj && this.purpose === 'edit') {
      if (this.dangKyTanThuDvhcIOForm.value.xacombobox) {
        this.dangKyTanThuDvhcIOForm.controls.xa.setValue({
          idxa: this.dangKyTanThuDvhcIOForm.value.xacombobox.idxa,
          maxa: this.dangKyTanThuDvhcIOForm.value.xacombobox.maxa
        });
        this.tenXaDisplay = this.dangKyTanThuDvhcIOForm.value.xacombobox.tenxa;
      } else {
        if (this.dangKyTanThuDvhcIOForm.value.tinhcombobox || this.dangKyTanThuDvhcIOForm.value.huyencombobox) {
          this.dangKyTanThuDvhcIOForm.controls.xa.setValue("");
          this.tenXaDisplay = "";
  ...
  private addOrUpdate(operMode: string) {
    const dkTanThuDvhcService = this.dangKyHoatDongKhoangSanFacadeService.getDangKyTanThuDvhcService();
    // Gán dữ liệu input vào model
    const idtinh = this.dangKyTanThuDvhcIOForm.value.tinh.idtinh;
    const idhuyen = this.dangKyTanThuDvhcIOForm.value.huyen.idhuyen;
    const idxa = this.dangKyTanThuDvhcIOForm.value.xa.idxa;
    this.inputModel.matinh = this.dangKyTanThuDvhcIOForm.value.tinh.matinh;
    this.inputModel.mahuyen = this.dangKyTanThuDvhcIOForm.value.huyen.mahuyen;
    this.inputModel.maxa = this.dangKyTanThuDvhcIOForm.value.xa.maxa;
    ...
    if (operMode === "new") {
      dkTanThuDvhcService.addItem(this.inputModel).subscribe(
        (res) => this.matSidenavService.doParentFunction("getAllDangKyTanThuDvhc"),
      ...
      dkTanThuDvhcService.updateItem(this.inputModel).subscribe(
        (res) => this.matSidenavService.doParentFunction("getAllDangKyTanThuDvhc"),
        (error: HttpErrorResponse) => {
          this.commonService.showDialogWarning(error.error.errors);
        },
  ...
  async onSubmit(operMode: string) {
    ...
    if (this.dangKyTanThuDvhcIOForm.valid === true) {
  ...
  public onFormReset() {
    ...
    this.dangKyTanThuDvhcIOForm.reset();
  }
   ...
   * Hàm close sidenav
   */
  public closeDkTanThuDvhcIOSidenav() {
    this.matSidenavService.close();
  }
