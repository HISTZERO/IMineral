...
@Component({
  selector: 'app-dongcuamo-io',
  templateUrl: './dongcuamo-io.component.html',
  styleUrls: ['./dongcuamo-io.component.scss']
})
export class DongcuamoIoComponent implements OnInit {

  @ViewChild('dangKyDongCuaMoTabs', {static: false}) dangKyDongCuaMoTabs;
  ...
  @ViewChild("thongTinDangKyComp", {static: false}) thongTinDangKyComp: DcmThongtindangkyComponent;
  // Chứa dữ liệu menu item trên subheader
  public navArray = MenuDkDongCuaMoChitiet;

  // Chứa dữ liệu nút quay lại trên subheader
  public btnArray = ButtonBackDongCuaMo;
  ...
  async ngOnInit() {
    ...
    this.dangKyDongCuaMoTabs.selectedIndex = this.TabType.ThongTinHoSo;
    this.dangKyDongCuaMoTabs.realignInkBar();
  }
  ...
  private async getDangKyThamDoByIdHoSo(loaiCapPhep: string, idHoSo: string) {
    if (loaiCapPhep === LoaiCapPhepEnum.DongCuaMotPhanDienTichKhuVucKhaiThacKhoangSan) {
      const dkThamDoKhoangSanService = this.dangKyHoatDongKhoangSanFacadeService.getDangKyDongCuaMoService();
      const dangKyItem = await dkThamDoKhoangSanService.getDangKyDongCuaByIdHoSo(idHoSo).toPromise();
      return dangKyItem;
    } else if (loaiCapPhep === LoaiCapPhepEnum.DongCuaMoKhoangSan) {
      const dkThamDoKhoangSanService = this.dangKyHoatDongKhoangSanFacadeService.getDangKyDongCuaMoService();
      const dangKyItem = await dkThamDoKhoangSanService.getDangKyDongCuaByIdHoSo(idHoSo).toPromise();
      return dangKyItem;
    }
